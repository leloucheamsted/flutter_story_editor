...
class FiltersView extends StatefulWidget {
  final List<List<double>>
      selectedFilters; // List holding currently selected filters for each page.
  final List<File>?
      selectedFiles; // Optional list of media files to apply filters on.
  final int
      currentPageIndex; // Index to identify the current page or media file.
  final Function(List<double>)
      onFilterChange; // Callback function triggered on changing a filter.
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
          color:
              darkGreenColor), // Background decoration of the filter container.
      child: ListView.builder(
        padding: const EdgeInsets.only(right: 10, left: 5),
        itemCount: Consts.filters
            .length, // Number of filters available from a constant source.
        scrollDirection:
        ...
        itemBuilder: (context, index) {
          return GestureDetector(
            onTap: () {
              setState(() {
                widget.selectedFilters[widget.currentPageIndex] = Consts
                        .filters[
                    index]; // Updates the selected filter for the current page.
              });
              widget.onFilterChange(widget.selectedFilters[widget
                  .currentPageIndex]); // Triggers the callback with the new filter selection.
            },
            child: Container(
                      ...
                      ? Align(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(width: 1.5, color: tealColor),
                              color: tealColor,
                            ),
                            child: const Center(
