...
class MainControlsView extends StatefulWidget {
  ...
  final List<File>
      uiViewEditableFiles; // List of editable media files for the UI.
  final List<List<double>>
      selectedFilters; // List of filters applied to each media file.
  final List<List<DraggableTextWidget>>
      textList; // Lists of draggable text widgets for each page.
  final List<List<DraggableStickerWidget>>
      stickerList; // Lists of draggable sticker widgets for each page.
  ...
  final VoidCallback
      onPaintClickListener; // Callback for activating paint mode.
  final VoidCallback onTextClickListener; // Callback for activating text mode.
  final VoidCallback
      onStickersClickListener; // Callback for activating stickers mode.
...
class _MainControlsViewState extends State<MainControlsView> {
  final Map<File, Uint8List?> _thumbnails =
      {}; // Cache for storing generated thumbnails for video files.
        ...
        children: [
          ...
          Column(
            children: [
              ...
              else if (widget.isFocused == false)
                CaptionView(
